...
class SessionBreakOverlay extends ConsumerStatefulWidget {
  const SessionBreakOverlay({super.key});

  @override
  ConsumerState<SessionBreakOverlay> createState() => _SessionBreakOverlayState();
}

class _SessionBreakOverlayState extends ConsumerState<SessionBreakOverlay> {
  @override
  Widget build(BuildContext context) {
    ...
    if (!ref.read(_breakNotifiedProvider)) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (!mounted) return;
        ref.read(_breakNotifiedProvider.notifier).state = true;
        ref.read(localNotificationsProvider.notifier).push(
              LqNotification(
                id: 'local_break_${duration.inMinutes}',
                title: 'Time for a short break?',
                body: 'You\'ve been learning for ${duration.inMinutes} minutes.',
                kind: LqNotificationKind.breakTime,
